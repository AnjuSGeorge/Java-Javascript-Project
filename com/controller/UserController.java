...
public class UserController {
    ...
    public String login(HttpServletRequest request, User user, ModelMap map) {  // http://ip:port/appname/login.html
    	...
    	if (userDAO.validateUser(user)) {
           ...
           // request.getSession().setAttribute("user", user);
            ...
            request.getSession().setAttribute("Emailid", user.getEmail());
    ...
    {  
    	...
    	boolean flag  = userDAO.updateProfile(user,request,insertPic);
        if (flag) {
    ...
    public String CreateLeague(HttpServletRequest request, User user, ModelMap map) {  // http://ip:port/appname/createNewLeague.html
	...
	public String InviteUsers(HttpServletRequest request, User user, ModelMap map)
	{
		...
		String tokens=leagueName+"_"+LeagueId;
		 String MessageContent =" Lucky you."+invitee+" has invited you to join their custom league  "+leagueName+"  http://localhost:8080/DraftLeague_new/createMyTeam/"+tokens;
    ...
    public String CreateNewLeague(@PathVariable(value="name") String name, ModelMap map) {
	 ...
	 @RequestMapping(value="/createMyTeam/{name}", method= RequestMethod.GET)
	    public String createTeam(@PathVariable(value="name") String name, ModelMap map,HttpServletRequest request) {
		 System.out.println("MyTeam:" + userDAO.checkSession(request));
		    if ( userDAO.checkSession(request)) {
		   
		 		System.out.println("Leaguenameandid"+name);
				map.addAttribute("LeagueNameId", name);
	 ...
	 @RequestMapping(value = "/createMyTeam", method = RequestMethod.POST)
	    public String CreateTeam(HttpServletRequest request, User user, ModelMap map) {  // http://ip:port/appname/register.html
	    	int teamId  = userDAO.createMyTeam(user,request);
	    	System.out.println("Team Id"+teamId);
	        if (teamId != 0) {
	            System.out.println("Insertion success");
	            return "afterTeamCreation";
	        } 
	        else
	            map.addAttribute("error", "Operation failed...");
	       // map.addAttribute("countryList", locationController.countryList(false));
	 ...
	 @RequestMapping(value = "/myteam", method = RequestMethod.GET)
	    public String myTeam(HttpServletRequest request, User user, ModelMap map) {  // http://ip:port/appname/myteam.html
	        //boolean SessionExt = userDAO.checkSession(request);
	    System.out.println("MyTeam:" + userDAO.checkSession(request));
	    if ( userDAO.checkSession(request)) {
